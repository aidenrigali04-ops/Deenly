...
export function ProfilePageShell({ children, variant = "default" }: ProfilePageShellProps) {
  const footerPad = variant === "compactFooter" ? "pb-6" : "pb-10";
  return (
    <section className="mx-auto w-full max-w-4xl">
      <article
        className={`surface-card rounded-b-2xl border border-black/10 px-4 pt-6 shadow-soft md:px-8 ${footerPad}`}
      >
        {children}
      </article>
    </section>
